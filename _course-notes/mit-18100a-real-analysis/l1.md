...
$$\forall\epsilon>0,\ \exists N\in\mathbb{N},\ \forall n\ge N:\ \left|\frac{n^{2}}{n^{2}+n+1}-1\right|<\epsilon .$$
...
**示例（正确使用 $\exists!$）。** $\forall a\in\mathbb{R},\ \exists!\,b\in\mathbb{R}: a+b=0$——存在（取 $b=-a$）且唯一（若 $a+b=a+b'=0$ 则 $b=b'$）。
...
3. **有理数（rational numbers）**：$\mathbb{Q}=\left\{\frac mn\ \middle|\ m,n\in\mathbb{Z}\ \text{且}\ n\neq0\right\}$。
