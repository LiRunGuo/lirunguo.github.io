...
$$n\ge M\ \Longrightarrow\ |a_n-L|<\epsilon .$$

对足够大的 $k$ 有 $n_{k-1}+1\ge M$，故 $|a_{n_{k-1}+1}-L|<\epsilon$，即 $\lim_{k\to\infty}a_{n_{k-1}+1}=L$。又

$$\left|\left(a_{n_{k-1}+1}-\frac1k\right)-L\right|\le\left|a_{n_{k-1}+1}-L\right|+\frac1k\ \longrightarrow\ 0,$$
...
可以看到从 $m=4$ 起即进入 $\frac12$ 邻域，此后一直留在其中（$|x_{20}-1|=0.05$、$|x_{100}-1|=0.01$，越往后越靠近 $1$）。
...
$$n\ge M\ \Longrightarrow\ |x_n-x|<\epsilon,\qquad\text{即}\qquad x-\epsilon<x_n<x+\epsilon .$$
...
**数值验算（方向 $\Rightarrow$）.** 取 $x_n=(-1)^n/n\to x=0$，取 $\epsilon=0.01$。收敛定义给出阈值：$|x_n|\le\frac1n<0.01\iff n>100$，故 $M=101$。核对 $n=101$：尾部集合为 $\{(-1)^k/k\mid k\ge101\}$，其 $\inf\approx-1/102=-0.00980392$，$\sup\approx1/101=0.00990099$，二者都在 $(-0.01,0.01)$ 内。而本例的 $\liminf=\limsup=0$（Exercise 2.3.5，见"与教材的对应"）。
...
$$\Lambda:=\Bigl\{y\in\mathbb{R}\ \Big|\ \text{存在子序列}\ \{x_{n_k}\}\ \text{使}\ \lim_{k\to\infty}x_{n_k}=y\Bigr\}$$
...
**证明.** $\{x_n\}$ 有界、$\{y_n\}$ 有界：取 $B_1,B_2\ge0$ 使 $|x_n|\le B_1$、$|y_n|\le B_2$ 对一切 $n$ 成立，则 $|x_n+y_n|\le|x_n|+|y_n|\le B_1+B_2$，故 $x_n+y_n$ 有界（这是 Assignment 5 第 7 题 (a) 的内容，见"与教材的对应"）。
...
##### （补充）Assignment 5 第 5 题：$\lim x_n=0\iff\limsup|x_n|=0$

**命题 E.** 设 $\{x_n\}$ 是有界序列。则

$$\lim_{n\to\infty}x_n=0\iff\limsup_{n\to\infty}|x_n|=0 .$$

**证明（$\Rightarrow$）.** 设 $x_n\to0$。由 $\bigl||x_n|-0\bigr|=|x_n-0|$，得 $|x_n|\to0$，即 $\{|x_n|\}$ 收敛到 $0$。由 Theorem 105，收敛序列的 $\limsup$ 等于其极限，故 $\limsup|x_n|=0$。

**证明（$\Leftarrow$）.** 设 $\limsup_{n\to\infty}|x_n|=0$。因为 $|x_n|\ge0$ 对一切 $n$ 成立，故每个尾部集合 $\{|x_k|\mid k\ge n\}$ 中的元素都 $\ge0$，于是其下确界 $\ge0$，即

$$\liminf_{n\to\infty}|x_n|=\lim_{n\to\infty}\inf\{|x_k|\mid k\ge n\}\ge0 .$$

（这里"下确界 $\ge0$ 对每个 $n$ 成立"→"极限 $\ge0$"用了 Lecture 8 的极限保持非严格不等式。）又由 Theorem 98 第②条 $\liminf|x_n|\le\limsup|x_n|=0$。合起来

$$0\le\liminf_{n\to\infty}|x_n|\le0\ \Longrightarrow\ \liminf_{n\to\infty}|x_n|=\limsup_{n\to\infty}|x_n|=0 .$$

由 Theorem 105，$\{|x_n|\}$ 收敛，且极限为 $0$。最后由 $\bigl|x_n-0\bigr|=|x_n|\to0$ 得 $x_n\to0$。$\blacksquare$

**数值验算.** 取 $x_n=\frac{(-1)^n}{n}$。则 $|x_n|=\frac1n$：$n=10$ 时 $x_{10}=0.1$、$|x_{10}|=0.1$；$n=100$ 时 $x_{100}=0.01$、$|x_{100}|=0.01$；$n=1000$ 时 $x_{1000}=0.001$、$|x_{1000}|=0.001$。$\limsup|x_n|=0$ 与 $\lim x_n=0$ 同时成立，符合命题。

**【证明机制解说】** （$\Rightarrow$）直接搬运绝对值的连续性（$\bigl||a|-|b|\bigr|\le|a-b|$）与 Theorem 105。（$\Leftarrow$）的机制是"**用非负性把 $\liminf$ 顶到 $0$**"：非负序列的 $\liminf\ge0$ 是免费的，配合已知的 $\limsup=0$ 与 $\liminf\le\limsup$，立刻逼出 $\liminf=\limsup=0$，再用 Theorem 105 收尾。

**【证明技巧总结】** （a）"$\limsup=0$ 型"命题通常要**同时使用 $\liminf$**：单靠 $\limsup=0$ 不足以说明序列收敛到 $0$（因为 $\liminf$ 可能更小，虽然本例中非负性排除了这种可能）；（b）证明非负序列的 $\liminf\ge0$ 时要点明"$0$ 是每个尾部的下界"；（c）把"$|x_n|\to0$"翻译回"$x_n\to0$"时写出 $|x_n-0|=|x_n|$，避免口头带过；（d）本命题是"用 $\limsup$ 判断收敛"的第一个实用范例，说明 $\limsup$ 可以充当**收敛性的探针**。
...
**逐题对应（每题一句话）.**
1. **Exercise 2.2.9**：设 $x_n\ne x$ 且比值极限 $L:=\lim_{n\to\infty}\frac{|x_{n+1}-x|}{|x_n-x|}$ 存在且 $L<1$，证明 $x_n\to x$；证明思路是取 $L<r<1$，由比值最终 $<r$ 得 $|x_{n+1}-x|<r|x_n-x|$，归纳得 $|x_n-x|\le r^{\,n-M}|x_M-x|$，再用 $r^n\to0$（Lecture 8 的几何序列极限）与夹逼定理。
...
4. **Exercise 2.3.7**：(a) 两个有界序列之和仍有界（用三角不等式 $|x_n+y_n|\le|x_n|+|y_n|$）；(b) 证明 $\limsup(x_n+y_n)\le\limsup x_n+\limsup y_n$ 并举出严格不等号的例子（$x_n=(-1)^n$、$y_n=(-1)^{n+1}$ 给出 $0<2$）；这正是本讲的**命题 D**。
5. **第 5 题**：设 $\{x_n\}$ 是有界序列，证明 $\lim_{n\to\infty}x_n=0\iff\limsup_{n\to\infty}|x_n|=0$；这就是本讲的**命题 E**，正向用 Theorem 105，反向用非负性给出 $\liminf\ge0$ 再配合 $\liminf\le\limsup$。
...
7. **Exercise 2.4.8**：判断真假并证明或给出反例——"若 $\{x_n\}$ 是 Cauchy 序列，则存在 $M$ 使得对一切 $n\ge M$ 有 $|x_{n+1}-x_n|\le|x_n-x_{n-1}|$"；**命题为假**，取 $x_n=\frac{\sin(\ln n)}{\sqrt{n}}$（$n\ge2$），它作为收敛到 $0$ 的序列是 Cauchy 序列（因为 $\frac{1}{\sqrt n}\to0$，由 Lecture 10 的"收敛 $\Rightarrow$ Cauchy"），但相邻差并不最终单调不增：$|x_4-x_3|=0.022661$，$|x_5-x_4|=0.044634$，出现上升，且波动型振荡在整个序列中反复出现，因此不存在这样的 $M$。
...
6. **本讲（$\limsup$ 的次可加性与比较原理）→ Lecture 12（比值判别法与根值判别法）.** 级数判别法的精细版本用 $\limsup$ 表达：例如根值判别法断言 $\limsup|a_n|^{1/n}<1\Rightarrow\sum a_n$ 绝对收敛，用到的正是"$\limsup$ 在乘法/幂运算下的不等式性质"（$\limsup(x_ny_n)\le(\limsup x_n)(\limsup y_n)$ 型估计）以及本讲的比较原理（命题 C）；而比值型判别法的最强形式（Lebl 2.3.15）也以 $\limsup$ 表述。**本讲的 $\limsup$ 是把"极限不存在"的序列也纳入判别范围的关键工具**。
