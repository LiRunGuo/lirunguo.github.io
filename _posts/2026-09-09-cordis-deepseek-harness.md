---
title: "[转载] Cordis：从开源 QQ 机器人内核到 DeepSeek Harness 平台底座"
date: 2026-09-09
permalink: /posts/2026/09/cordis-deepseek-harness/
author: Lss233
author_profile: false
excerpt: "转载 Lss233 的 Cordis 技术文章：从插件生命周期、可逆副作用与服务注入，到 DeepSeek Harness 的插件架构及其理论基础。"
tags:
  - Cordis
  - DeepSeek Harness
  - AI Agents
...
本文转载收录于 2026 年 9 月 9 日，原作者为 **Lss233**。原文信息及来源链接保留如下。

> **作者**：Lss233（腾讯 · 技术研发）
> **发布时间**：2026-08-18 11:27
> **最后编辑**：2026-08-19 11:22
> **原文链接**：<https://cloud.tencent.cn/developer/article/2728030>
> **标签**：agent、DeepSeek
...
### 1.2 作者：Shigma（Yifan Shi）

先认识一下写出它的人——**Shigma（Yifan Shi）**。

Shigma 在 QQ 机器人圈子里是相当出名的存在。他的 GitHub 账号（`shigma`）下有 130 多个公开仓库，如果你把 npm 上几个包的主页打开，会看到一长串 maintainer 都是同一个人：
...
如今，Cordis 的配套论文《A Programming Paradigm for Spatiotemporal Composability》（第 5 章的主角）以预印本发布，署名单位是北京大学与 DeepSeek-AI，第一作者 Yifan Shi，合著者 Wei Zhang、Tianyi Cui。论文把 Cordis 的机制形式化，给出了定义、定理与证明。
...
Cordis 的配套论文《A Programming Paradigm for Spatiotemporal Composability》（[cordiverse/paper](https://github.com/cordiverse/paper)）由北京大学与 DeepSeek-AI 的研究者合著，88 页预印本，第一作者 Yifan Shi 的经历见 1.2。它做的事不是"给 Cordis 写说明书"，而是**把 Cordis 的核心机制形式化，证明它们构成一种新的编程范式**。论文里没有一句"我们的框架很优秀"，有的是定义、定理与证明，以及一个贯穿始终的主张：

> **动态组合（运行时装卸组件）缺乏形式基础；Cordis 用两个运行时机制把它补上了，并证明了由此得到的性质。**

定理编号（Theorem、Corollary、Definition）都对应论文原文，本文只讲直觉。
...
- 作者信息（2026-08 核实）：GitHub 用户 `shigma`（132 个公开仓库，圈内昵称"梦梦"）；npm maintainer 为 `shigma` 的包包括 `koishi`、`cordis`、`@satorijs/core`、`schemastery`、`minato`、`cosmokit`、`@koishijs/core`；论文《A Programming Paradigm for Spatiotemporal Composability》署名：Yifan Shi（北京大学）、Wei Zhang 与 Tianyi Cui（DeepSeek-AI）
...
- 社区插件生态：[awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin)（GitHub 话题 [`dsh-plugin`](https://github.com/topics/dsh-plugin)，2026-08 收录 174 个插件）
...
- 文中引用的 DSH 代码与配置文件，来自本机安装的 `@deepseek-ai/dsh` 0.1.0-rc.6 及其 `node_modules` 中的 `dsh-app-boot`、`dsh-base`、`dsh-goal`、`dsh-tool-cordis` 等包
